...
from vint_train.visualizing.action_utils import (
    visualize_traj_pred,
...
def get_obs_image(obs_image, obs_type, transform, device):
    if obs_type == "image":
        obs_images = torch.split(obs_image, 3, dim=1)
        viz_obs_image = TF.resize(obs_images[-1], VISUALIZATION_IMAGE_SIZE)
        obs_images = [transform(obs_image).to(device) for obs_image in obs_images]
        obs_image = torch.cat(obs_images, dim=1)
    elif obs_type == "image_mask_enc":
        viz_obs_image, obs_image = obs_image.split([3, obs_image.shape[1] - 3], dim=1)
        obs_image = TF.resize(obs_image, 60, 0).to(device)
        viz_obs_image = TF.resize(viz_obs_image, VISUALIZATION_IMAGE_SIZE)
    elif obs_type == "disabled":
        obs_image = torch.zeros_like(obs_image)
        viz_obs_image = TF.resize(obs_image, VISUALIZATION_IMAGE_SIZE)
    else:
...
def get_goal_image(goal_image, goal_type, transform, device, obs_image=None):
    ...
    elif goal_type == "image_mask_enc":
        viz_goal_image, goal_image = goal_image.split(
            [3, goal_image.shape[1] - 3], dim=1
        )
        goal_image = TF.resize(goal_image, 60, 0).to(device)
        viz_goal_image = TF.resize(viz_goal_image, VISUALIZATION_IMAGE_SIZE)
    elif goal_type == "disabled":
        goal_image = torch.zeros_like(obs_image)
    else:
...
def _log_data(
    ...
    **kwargs,
    ...
    if image_log_freq != 0 and i % image_log_freq == 0:
        if kwargs.get("predict_dists", True):
            visualize_dist_pred(
                to_numpy(obs_image),
                to_numpy(goal_image),
                to_numpy(dist_pred),
        ...
        visualize_traj_pred(
            ...
            (
                to_numpy(goal_image)
                if kwargs.get("goal_type") != "disabled"
                else to_numpy(obs_image)
            ),
